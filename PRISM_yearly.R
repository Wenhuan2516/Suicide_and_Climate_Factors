...
# Load PRISM overall data
 suicide_month <- read.csv('~/Desktop/county_by_month/county_to_year_PRISM_overall_2000_2019.csv')
...
varmean <- weighted.mean(suicide_month$suicide_rate, w = suicide_month$meanpop)

##############################Model1########################################
# Estimate model with felm (fixed effects lm)
form <- as.formula(paste('suicide_rate ~', paste(temp_bins, collapse = '+'), '+ lowp + highp | fips + year | 0 | statefips'))
